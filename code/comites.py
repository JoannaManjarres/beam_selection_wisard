...
def committe():
    ...
    #accuracy_position_geometric_mean, name_position_geometric_mean = committe_position_geometric_mean(beam_selection_for_coord, beam_selection_for_lidar, label_validation=index_beam_combined_test)
    #accuracy_difference_between_position, name_difference_between_position = committe_difference_between_position(beam_selection_for_coord, beam_selection_for_lidar, label_validation=index_beam_combined_test)
    #accuracy_position_add, name_position_add = committe_position_add(beam_selection_for_coord, beam_selection_for_lidar, label_validation=index_beam_combined_test)
    #accuracy_position_dot, name_position_dot = committe_position_dot(beam_selection_for_coord, beam_selection_for_lidar, label_validation=index_beam_combined_test)
    accuracy_bigest_score, name_bigest_score = committe_bigger_score(beam_selection_for_coord, beam_selection_for_lidar, label_validation=index_beam_combined_test)
...
def committe_position_dot(beam_selection_for_coord, beam_selection_for_lidar, label_validation):
    number_of_samples = len(beam_selection_for_coord)
        ...
        for i in range(len(list_of_class_of_beam_lidar)):
            for index, value in enumerate(list_of_class_of_beam_coord):
                ...
                if int(beam_lidar[i]['class']) == value:
                    index_of_class_in_coord = index
                    index_of_class_in_lidar = i
                    geometric_mean_of_position_per_class = index_of_class_in_coord * index_of_class_in_lidar
                    classe = int(beam_lidar[i]['class'])
                    list_of_classes_degree.append([classe, geometric_mean_of_position_per_class])
                    list_of_classes_degree_1.append((classe, geometric_mean_of_position_per_class))

        order_list_of_classes = sorted(list_of_classes_degree, key=itemgetter(1), reverse=False)
    ...
    name_committe = 'produto_da_posicao'
    df_acuracia_comite_top_k = pd.DataFrame(acuracia)
    path = '../results/accuracy/8X32/committe/'
    df_acuracia_comite_top_k.to_csv(path + 'acuracia_comite_' + name_committe + '_top_k.csv')
    ...
    return acuracia, name_committe
def committe_difference_between_position(beam_selection_for_coord, beam_selection_for_lidar, label_validation):
    number_of_samples = len(beam_selection_for_coord)
        ...
        for i in range(len(list_of_class_of_beam_lidar)):
            for index, value in enumerate(list_of_class_of_beam_coord):
                ...
                if int(beam_lidar[i]['class']) == value:
                    index_of_class_in_coord = index
                    index_of_class_in_lidar = i
                    geometric_mean_of_position_per_class = abs(index_of_class_in_coord - index_of_class_in_lidar)
                    classe = int(beam_lidar[i]['class'])
                    list_of_classes_degree.append([classe, geometric_mean_of_position_per_class])
                    list_of_classes_degree_1.append((classe, geometric_mean_of_position_per_class))
    ...
    name_committe = 'diferenca_da_posicao'
    df_acuracia_comite_top_k = pd.DataFrame(acuracia)
    path = '../results/accuracy/8X32/committe/'
    df_acuracia_comite_top_k.to_csv(path + 'acuracia_comite_' + name_committe + '_top_k.csv')
    ...
    return acuracia, name_committe
def committe_position_add(beam_selection_for_coord, beam_selection_for_lidar, label_validation):
    number_of_samples = len(beam_selection_for_coord)
        ...
        for i in range(len(list_of_class_of_beam_lidar)):
            for index, value in enumerate(list_of_class_of_beam_coord):
                ...
                if int(beam_lidar[i]['class']) == value:
                    index_of_class_in_coord = index
                    index_of_class_in_lidar = i
                    add_of_position_per_class = index_of_class_in_coord + index_of_class_in_lidar
                    classe = int(beam_lidar[i]['class'])
                    list_of_classes_degree.append([classe, add_of_position_per_class])
                    list_of_classes_degree_1.append((classe, add_of_position_per_class))
    ...
    return acuracia, name_committe
def committe_position_geometric_mean(beam_selection_for_coord, beam_selection_for_lidar, label_validation):
        ...
        for i in range(len(list_of_class_of_beam_lidar)):
            for index, value in enumerate(list_of_class_of_beam_coord):
                ...
                if int(beam_lidar[i]['class']) == value:
                    index_of_class_in_coord = index
                    index_of_class_in_lidar = i
                    geometric_mean_of_position_per_class = np.sqrt(index_of_class_in_coord * index_of_class_in_lidar)
                    classe = int(beam_lidar[i]['class'])
                    list_of_classes_degree.append([classe, geometric_mean_of_position_per_class])
                    list_of_classes_degree_1.append((classe, geometric_mean_of_position_per_class))
    ...
    name_committe = 'media_geometrica_da_posicao'
    df_acuracia_comite_top_k = pd.DataFrame(acuracia)
    path = '../results/accuracy/8X32/committe/'
    df_acuracia_comite_top_k.to_csv(path + 'acuracia_comite_' + name_committe + '_top_k.csv')
...
def committe_score_geometric_mean(beam_selection_for_coord, beam_selection_for_lidar, label_validation):
    ...
    name_committe = 'media_geometrica_do_score'
    df_acuracia_comite_top_k = pd.DataFrame(acuracia)
    path = '../results/accuracy/8X32/committe/'
    df_acuracia_comite_top_k.to_csv(path + 'acuracia_comite_' + name_committe + '_top_k.csv')
...
def committe_bigger_score(beam_selection_for_coord, beam_selection_for_lidar, label_validation):
    ...
    for i in range(number_of_samples):
        ...
        for i in range(len(list_of_class_of_beam_lidar)):
            for index, value in enumerate(list_of_class_of_beam_coord):
                # if beam_lidar[i]['class'] == value:
                if int(beam_lidar[i]['class']) == value:
                    if beam_coord[index]['norm'] > beam_lidar[i]['norm']:
                        list_of_classes.append([value, beam_coord[index]['norm']])
                    else:
                        list_of_classes.append([value, beam_lidar[i]['norm']])
...
def plot_results_from_csv():
    ...
    file_score_biggest = 'acuracia_comite_maior_do_score_top_k'
    file = path + file_score_dot + '.csv'
    data = pd.read_csv(file, usecols=[1])
    data_of_score_biggets = data.to_numpy()
    name_committe_biggest_score = 'Score: o Maior'
    ...
    plt.plot(top_k, data_of_score_biggets, color='lightcoral', linestyle='solid',
             label=name_committe_biggest_score, linewidth=8, alpha=.5)  # linestyle = 'solid'
    plt.plot(top_k, data_of_score_committe, 'ro')

    plt.plot(top_k, data_of_postion_committe, color='g', linestyle='dashed',
    ...
    plt.plot(top_k, data_of_score_dot, color='m', linestyle='dashed',
             label=name_committe_dot_score, linewidth=.8)  # linestyle = 'solid'
...
def read_result_csv(file_name):
    ...
    return data_of_postion_committe
def plot_results_in_bars_from_csv_position():
    top_k = [1, 5, 10, 20, 30, 40, 50]

    file_postion_media = 'acuracia_comite_media da posicao_top_k'
    name_committe_media_position = 'Posição: media aritmetica'
    data_of_postion_media = read_result_csv(file_postion_media)

    file_position_geometric_mean = 'acuracia_comite_media_geometrica_da_posicao_top_k'
    name_committe_media_geometrica_position = 'Posição: media geometrica'
    data_of_position_media_geometrica = read_result_csv(file_position_geometric_mean)

    file_position_add = 'acuracia_comite_soma_da_posicao_top_k'
    name_committe_add_position = 'Posição: soma'
    data_of_position_add = read_result_csv(file_position_add)

    file_position_difference = 'acuracia_comite_diferenca_da_posicao_top_k'
    name_committe_difference_position = 'Posição: Diferença'
    data_of_postion_difference = read_result_csv(file_position_difference)

    file_position_dot = 'acuracia_comite_produto_da_posicao_top_k'
    name_committe_dot_position = 'Posição: produto'
    data_of_position_dot = read_result_csv(file_position_dot)
    ...
    title_figure = "comparacao de Desempenho de comites: Posição"
    ...
    plt.subplots_adjust(right=0.796, left= 0.074)

    plt.grid(True, color="white", linewidth="1", alpha=0.5)
    #plt.box(False)


    # creating the bar plot
    bars_position = ax.bar(test, data_of_postion_media, color='maroon', width=width, label=name_committe_media_position)
    for bar in bars_position:
        height = bar.get_height()
        ax.annotate('{:.3}'.format(height),
                    xy=(bar.get_x() + bar.get_width() / 2, height-var_heigh),
                    fontsize=size_font, rotation=90, ha='center', va='bottom', color='w')

    bars_score_dot = ax.bar([i+1+var for i in test], data_of_position_media_geometrica, color='lightyellow', width=width, label=name_committe_media_geometrica_position)
    ...
    bars_score_difference = ax.bar([i-1-var for i in test], data_of_position_dot, color='steelblue', width=width, label=name_committe_dot_position)
    ...
    bars_score_add = ax.bar([i+2+var+var for i in test], data_of_position_add, color='c', width=width, label=name_committe_add_position)
    ...
    bar_score_media_geometrica = ax.bar([i+3+var*3 for i in test], data_of_postion_difference, color='lightsalmon', width=width, label=name_committe_difference_position)
    ...
    ax.set_xlabel('top-k')
    ax.set_title('Comparacao entre comites: posição')
    ax.set_xticks(test, top_k)
    ax.set_facecolor("#E2E8ED")
   ...
   # plt.bar(top_k, data_of_score_dot, color='maroon', width=0.4, label=name_committe_dot_score)
    ...
    plt.savefig(path + 'comparacao_comites_posicao.png', dpi=300, bbox_inches='tight')
    plt.show()

    a=0

def plot_results_in_bars_from_csv_score():
    top_k = [1, 5, 10, 20, 30, 40, 50]

    file_score_media =  'acuracia_comite_media do Score_top_k'
    name_committe_media_score = 'Score: media aritmetica'
    data_of_score_media = read_result_csv(file_score_media)

    file_score_geometric_mean = 'acuracia_comite_media geometrica do Score_top_k'
    name_committe_media_geometrica_score = 'Score: media geometrica'
    data_of_score_media_geometrica = read_result_csv(file_score_geometric_mean)

    file_score_add = 'acuracia_comite_soma_do_score_top_k'
    name_committe_add_score = 'Posição: soma'
    data_of_score_add = read_result_csv(file_score_add)

    file_score_difference = 'acuracia_comite_diferenca_do_score_top_k'
    name_committe_difference_score = 'Score: Diferença'
    data_of_score_difference = read_result_csv(file_score_difference)

    file_score_dot = 'acuracia_comite_produto_do_score_top_k'
    name_committe_dot_score = 'Score: produto'
    data_of_score_dot = read_result_csv(file_score_dot)
    ...
    title_figure = "comparacao de Desempenho de comites: Score"
    ...
    # creating the bar plot
    bars_position = ax.bar(test, data_of_score_media, color='maroon', width=width, label=name_committe_media_score)
    for bar in bars_position:
        height = bar.get_height()
        ax.annotate('{:.3}'.format(height),
                    xy=(bar.get_x() + bar.get_width() / 2, height-var_heigh),
    ...
    bars_score_dot = ax.bar([i+1+var for i in test], data_of_score_media_geometrica, color='lightyellow', width=width, label=name_committe_media_geometrica_score)
    for bar in bars_score_dot:
        height = bar.get_height()
        ax.annotate('{:.3}'.format(height),
                    xy=(bar.get_x() + bar.get_width() / 2, height-var_heigh),
    ...
    bars_score_difference = ax.bar([i-1-var for i in test], data_of_score_dot, color='steelblue', width=width, label=name_committe_dot_score)
    for bar in bars_score_difference:
        height = bar.get_height()
        ax.annotate('{:.3}'.format(height),
                    xy=(bar.get_x() + bar.get_width() / 2, height-var_heigh),
    ...
    bars_score_add = ax.bar([i+2+var+var for i in test], data_of_score_add, color='c', width=width, label=name_committe_add_score)
    for bar in bars_score_add:
        height = bar.get_height()
        ax.annotate('{:.3}'.format(height),
                    xy=(bar.get_x() + bar.get_width() / 2, height-var_heigh),
    ...
    bar_score_media_geometrica = ax.bar([i+3+var*3 for i in test], data_of_score_difference, color='lightsalmon', width=width, label=name_committe_difference_score)
    for bar in bar_score_media_geometrica:
        height = bar.get_height()
        ax.annotate('{:.3}'.format(height),
                    xy=(bar.get_x() + bar.get_width() / 2, height-var_heigh),
    ...
    ax.set_ylabel('Acurácia')
    ax.set_xlabel('top-k')
    ax.set_title('Comparacao entre comites: score')
    ax.set_xticks(test, top_k)
    ax.set_facecolor("#E2E8ED")
    ax.set_axisbelow(True)
    ...
    plt.savefig(path + 'comparacao_comites_score.png', dpi=300, bbox_inches='tight')
    plt.show()
...
#plot_results_in_bars_from_csv_position()
#plot_results_in_bars_from_csv_score()
